---
title: "Self-hosted Runner の稼働状況をリアルタイムで監視できる GitHub CLI 拡張を作った"
emoji: "🖥️"
type: "tech"
...
![gh-runner-monitor の実行結果](/images/8230ff397f868e/screenshot.png)
...
sequenceDiagram
    ...
    CLI->>Runners: 1. Runner 一覧を取得
    Runners-->>CLI: Runner 情報（ID, ステータス, busy 等）

    CLI->>Runs: 2. in_progress の Workflow run を取得
    Runs-->>CLI: 実行中の Workflow run 一覧

    CLI->>Runs: 3. queued の Workflow run を取得
    Runs-->>CLI: 待機中の Workflow run 一覧

    loop 各 Workflow run に対して
        CLI->>Jobs: 4. Job 一覧を取得
        Jobs-->>CLI: Job 一覧（runner_id を含む）
    end

    Note over CLI: 5. Runner と Job を突き合わせて表示
```
