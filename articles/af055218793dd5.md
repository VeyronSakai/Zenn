...
GitHub Actions の `workflow_dispatch` イベントを使うと、ワークフローを手動で実行できます。デプロイ先の環境やバージョン番号などをパラメータとして受け取れるため、運用系のワークフローでは非常に便利な機能です。
...
`workflow_dispatch` は、ワークフローに入力パラメータを定義できます。以下のような定義が可能です。
...
この定義により、ワークフロー実行時に環境やバージョンを指定できます。

しかし、ワークフロー実行後の Job Summary には、これらの入力値が表示されません。後から「このワークフローはどの環境にデプロイしたのか」「どのバージョンを指定したのか」を確認するには、ワークフローの実行詳細画面まで遷移する必要があります。

特にチームで運用している場合、他のメンバーが実行したワークフローの設定を素早く確認したいケースは多いでしょう。
...
この Action は、`workflow_dispatch` の入力値を Job Summary にテーブル形式で表示します。GitHub Marketplace でも公開しています。
...
GitHub API を使ってワークフローファイルを直接取得するため、余分なステップを追加する必要がありません。
...
入力名だけでなく、ワークフローで定義した説明（description）も一緒に表示されるため、各パラメータの意味がすぐにわかります。
...
ワークフローを実行すると、Job Summary に以下のようなテーブルが表示されます。
...
これにより、ワークフローの実行結果を見るだけで、どのパラメータで実行されたかを即座に把握できます。
...
GitHub API を使って、実行中のワークフローファイル（YAML）を取得します。これにより、`actions/checkout` なしでワークフローの定義にアクセスできます。

### 2. 入力定義の解析

取得したワークフローファイルから、`workflow_dispatch.inputs` セクションをパースします。各入力パラメータの名前と説明（description）を抽出します。

### 3. 実際の入力値とのマッピング

GitHub Actions の `github.event.inputs` コンテキストから、実際に渡された入力値を取得します。ワークフロー定義の情報と組み合わせて、テーブルのデータを構築します。
...
- **チーム運用に便利**: 他のメンバーが実行したワークフローの設定もすぐに把握できる
