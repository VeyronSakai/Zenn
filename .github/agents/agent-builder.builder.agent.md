...
エージェント作成が完了したら、必ず以下の lint コマンドを実行し、エラーがあれば修正すること。

```bash
# textlint: 日本語文章のチェック
npx textlint docs/ drafts/ AGENTS.md

# markdownlint: Markdown フォーマットのチェック
npx markdownlint-cli AGENTS.md docs/ drafts/ .github/agents/<namespace>.*.agent.md
```

- すべてのエラーを解消してから作業完了とする
- 警告（warning）も可能な限り対応する